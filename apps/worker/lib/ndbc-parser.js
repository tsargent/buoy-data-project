 ...
 */
export function parseNDBCFile(content) {
    const lines = content.split("\n").filter((line) => !line.startsWith("#") && line.trim());
    return lines.slice(2).map((line) => {
        const parts = line.trim().split(/\s+/);
        const [year, month, day, hour, minute, waveHeight, dominantPeriod, averagePeriod, meanWaveDirection, pressure, airTemp, waterTemp, dewPoint, visibility, windDirection, windSpeed, gustSpeed,] = parts;
